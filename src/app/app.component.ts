...
import {ReaderTextService} from "./readerText-service";
...
export class AppComponent {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.resultadoPrediccion && this.resultadoPrediccion.nativeElement) {
      ...
      this.resultadoPrediccion.nativeElement.value = 'Inicializado correctamente';
    }
  }
  ...
  enviarArchivoYParametros(event: any) {
    ...
    const vectorAleatorio = this.generarArreglo();
  ...
  generarArreglo(): number[] {
    const arreglo: number[] = [];
    for (let i = 0; i < 63; i++) {
      const valor = Math.random(); // Genera un valor aleatorio entre 0 y 1
      arreglo.push(valor);
    }
    console.log(arreglo); // Imprime el arreglo generado en la consola
    // Aquí puedes hacer lo que quieras con el arreglo generado, como almacenarlo en una variable de clase
    return arreglo; // Asegúrate de devolver el vector correctamente
